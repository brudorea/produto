...
/* Executar no "reverse engineer <ctrl><r> e configurar para verificação da chave primária e chave estrangeira
 
Linha de relacionamento ligando as tabelas:
O risquinho duplo || indica chave primária (de onde saiu a chave estrangeira)
O risquinho diferente |< indica chave estrangeira (para onde foi a chave estrangeira)
 
Cores das linhas relacionadas a chave estrangeira e qual dado foi utilizado quando passa o mouse no nome da tabela:
Azul -> de onde saiu a chave estrangeira (quando deixa o mouse no nome da tabela)
Verde -> para onde foi a chave estrangeira (quando deixa o mouse no nome da tabela)
Amarela -> Informa que esses campos são chaves estrangeiras (quando deixa o mouse na linha de relacionamento que liga as tabelas)
 
Símbolos e cores nas tabelas:
Pela chave amarela -> exibe que trata-se da chave primária da tabela
Pela cor do losango azul -> exibe que trata-se de um campo com configuração especial
Pela cor do losango transparente -> exibe que trata-se de um campo sem configuração especial
Pela cor do losango rosa -> exibe que trata-se de chave estrangeira
*/
...
update alunos set cursoprefere = '1'
where id = '19';
...
select nome, ano from cursos;
select nome, cursoprefere from alunos;

select cursos.nome, cursos.ano, alunos.nome, alunos.cursoprefere
from cursos JOIN alunos
ON idcurso = cursoprefere;
...
from alunos LEFT OUTER JOIN cursos
ON idcurso = cursoprefere
order by alunos.nome;

select alunos.nome, cursos.nome, cursos.ano
from alunos RIGHT OUTER JOIN cursos
ON idcurso = cursoprefere
order by alunos.nome;
...
ON alunos.id = prefere.idalunos
JOIN cursos
ON cursos.idcurso = prefere.idcursos
order by alunos.nome;

select alunos.nome, cursos.nome, prefere.data from alunos
JOIN prefere
ON alunos.id = prefere.idalunos
JOIN cursos
ON cursos.idcurso = prefere.idcursos
